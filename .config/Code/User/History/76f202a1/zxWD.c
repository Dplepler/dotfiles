...
void bitmap_mark_kernel() {
    ...
    terminal_write_int(frame_bitmap, 16);

    size_t kernel_pages = (pgulong_t)(&_kernel_end - &_kernel_start) / 0x1000;  // Amount of pages to mark
    const uint32_t kernel_first_index = ((uint32_t)&_kernel_start) / 0x1000;    // Kernel's first bitmap index

    PRINT_NEW_LINE;
    terminal_write_int(((uint32_t)&_kernel_end) - ((uint32_t)&_kernel_start), 16);
    ...
    for (uint32_t i = 0; i < 10; i++) {
        terminal_write_int(frame_bitmap[kernel_first_index + i], 16);
        PRINT_NEW_LINE;
    }
}
