...
void bitmap_mark_kernel() {
    ...
    for (uint32_t i = 0; i <= kernel_pages; i++) {
        //frame_bitmap[((uint32_t)i + kernel_first_index) / BITS_IN_LONG] |=  (1 << ((uint32_t)i % BITS_IN_LONG));
        MARK_USED(frame_bitmap, (kernel_first_index + i));
    }

    terminal_write_int(frame_bitmap[0xC0000], 16);
    PRINT_NEW_LINE;

    terminal_write_int(frame_bitmap[0x6000], 16);
}
